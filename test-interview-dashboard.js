    ...
    try {
        ...
        await query(
            ...
            [
                ...
                80,
                85,
                JSON.stringify(['Clear communication', 'Good technical knowledge', 'Confident presentation']),
                JSON.stringify(['Practice more system design', 'Work on edge case handling']),
                JSON.stringify({ overallFeedback: 'Strong performance with room for improvement in system design' })
            ]
        );
        ...
        dashboardResult.rows.forEach((row, index) => {
            ...
            if (row.overall_score) {
                console.log(`      Score: ${row.overall_score}% (Comm: ${row.communication_score}%, Tech: ${row.technical_score}%, Conf: ${row.confidence_score}%)`);
            }
        });

        // Test 5: Calculate summary stats
        console.log('\n5. Testing summary calculations...');
        const completedInterviews = dashboardResult.rows.filter(row => row.status === 'completed' && row.overall_score);
        const avgScore = completedInterviews.length > 0 
            ? Math.round(completedInterviews.reduce((sum, row) => sum + row.overall_score, 0) / completedInterviews.length)
            : 0;
        ...
        const summary = {
            ...
            lastInterviewDate: dashboardResult.rows.length > 0 ? dashboardResult.rows[0].created_at : null
        };
        ...
        console.log(`   Last Interview: ${summary.lastInterviewDate ? new Date(summary.lastInterviewDate).toLocaleDateString() : 'None'}`);
