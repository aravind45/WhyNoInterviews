 ...
 */
export const initializeClaude = (): void => {
  const apiKey = process.env.ANTHROPIC_API_KEY;

  if (!apiKey || apiKey === 'your-anthropic-api-key-here') {
    logger.warn('ANTHROPIC_API_KEY not configured - Claude AI analysis will be unavailable');
    return;
  }
...
/**
 * Analyze resume and generate diagnosis using Claude
 */
export const analyzeResume = async (
  ...
  if (!claudeClient) {
    throw new ProcessingError('Claude AI analysis service is not configured');
  }
  ...
  try {
    ...
    if (!content || content.type !== 'text') {
      throw new ProcessingError('Empty response from Claude AI service');
    }
    ...
    throw new ProcessingError('Claude AI analysis failed. Please try again.');
  }
};
