...
async function finishInterview() {
  ...
  try {
    ...
    const response = await fetch(
      `/api/mock-interview/interview-results/${currentSession.sessionToken}`,
    );
    ...
    if (feedback.strengths && feedback.strengths.length > 0) {
      ...
      li.textContent =
        'No specific strengths identified. Keep practicing to improve your performance!';
      strengthsList.appendChild(li);
    }
...
function displayRubricScores(rubrics) {
  ...
  Object.entries(rubrics).forEach(([key, rubric]) => {
    ...
    const stars = Array.from(
      { length: 5 },
      (_, i) => `<span class="star ${i < rubric.score ? 'filled' : ''}">★</span>`,
    ).join('');
...
function displayDetailedFeedback(detailedFeedback) {
  ...
  container.innerHTML =
    '<h3 style="margin-bottom: 16px; color: #1e293b;">📝 Detailed Feedback</h3>';
