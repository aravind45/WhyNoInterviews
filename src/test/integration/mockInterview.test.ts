    ...
    return {
        getProvider: jest.fn().mockReturnValue({
            ...
            generateText: jest.fn().mockResolvedValue(JSON.stringify({
                overallScore: 85,
                rubrics: {
                    communication: { score: 4, feedback: 'Good', improvements: 'None', showInsights: false },
                    technicalKnowledge: { score: 5, feedback: 'Excellent', improvements: 'None', showInsights: false }
                },
...
describe('Mock Interview Integration Tests', () => {
    ...
    describe('End-to-End Mock Interview Flow', () => {
        ...
        it('should upload a response (mock video)', async () => {
            ...
            const res = await request(app)
                .post('/api/upload-interview-response')
                .send({
                    sessionToken,
        ...
        it('should analyze a response', async () => {
            ...
            // Analyze
            const res = await request(app)
                .post('/api/analyze-interview-video')
                .send({
                    sessionToken,
                    questionId: 1,
                    videoUrl: 'http://mock-storage.com/video.webm'
        ...
        it('should generate final results', async () => {
            ...
            // Generate results
            const res = await request(app)
                .get(`/api/interview-results/${sessionToken}`);
