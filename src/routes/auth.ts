...
async function linkSessionToUser(sessionId: string, userId: string) {
  ...
  try {
    ...
    await pool.query(`UPDATE user_sessions SET user_id = $1 WHERE session_id = $2`, [
      userId,
      sessionId,
    ]);
  } catch (err) {
...
router.post('/forgot-password', async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (user) {
      ...
      // Log to console (STUB)
      logger.info(
        `[STUB] Password Reset Link for ${email}: http://localhost:3000/reset-password?token=${token}`,
      );
    }
