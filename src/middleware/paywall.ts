...
export const paywallMiddleware = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const whitelistIps = (process.env.PAYWALL_WHITELIST_IPS || '')
      .split(',')
      .map((ip) => ip.trim());
    ...
    return res.status(402).json({
      ...
      message:
        'This feature is currently locked. Please contact the administrator for access or stay tuned for the public release.',
