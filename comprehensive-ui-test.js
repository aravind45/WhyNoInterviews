...
async function testNewUIMode() {
  ...
  try {
    ...
    console.log('   ✓ Home section visible after click:', homeActiveAfterClick.sectionVisible ? 'YES' : 'NO');
    ...
    const visualElements = await page.evaluate(() => {
      return {
        hasGradientBackground: window.getComputedStyle(document.body).background.includes('gradient'),
        headerIsWhite: window.getComputedStyle(document.querySelector('header')).backgroundColor === 'rgb(255, 255, 255)',
        logoHasOrangeGradient: window.getComputedStyle(document.querySelector('.logo span')).background.includes('gradient'),
        tabsHaveRoundedStyle: window.getComputedStyle(document.querySelector('.main-tabs')).borderRadius !== '0px'
      };
    });
